      ...
      <div class="flex flex-col gap-4 p-4 bg-workspace-panel border-t border-workspace-border text-xs text-workspace-text">
        ...
        <div class="flex items-center justify-between pt-2 border-t border-workspace-border">
          ...
          <div class="flex items-center gap-3">
            ...
            >
              ${isExporting ? `
                <svg class="animate-spin -ml-1 mr-1 h-3.5 w-3.5 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      if (this.selectedFormat === 'csv') {
        ...
        if (mode === 'contact-sheet') {
          ...
          const singlePage: any = {
            pageIndex: 0,
            totalPages: 1,
            canvasWidth: collageLayout.canvasWidth,
            canvasHeight: collageLayout.canvasHeight,
            cells: collageLayout.cells.map(c => ({
              image: c.image || targetImages[0],
              x: c.x,
              y: c.y,
              width: c.width,
              height: c.height,
              status: c.image?.status || 'unreviewed',
            })),
          };
          await exportContactSheetPagesToPDF([singlePage], config);
        }
        $exportProgress.set(100);
      } else {
        ...
        if (mode === 'contact-sheet') {
          ...
          if (activePage) {
            await exportLayoutAsImage(activePage, config, this.selectedFormat as 'png' | 'jpeg', this.quality, `frameproof-sheet-page-${currentPageIndex + 1}`);
          }
        } else {
          const template = COLLAGE_TEMPLATES.find(t => t.id === $activeTemplateId.get()) || COLLAGE_TEMPLATES[0];
          const collageLayout = calculateCollageLayout(targetImages, template, config, this.scale);
          await exportLayoutAsImage(collageLayout, config, this.selectedFormat as 'png' | 'jpeg', this.quality, `frameproof-collage-${template.id}`);
        }
        $exportProgress.set(100);
      }
    } catch (err) {
